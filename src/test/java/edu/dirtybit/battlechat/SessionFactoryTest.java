...
public class SessionFactoryTest {
    ...
    public void SessionFactory_ShouldNotReturnNull_WhenCreatingSessionFromConfigProvider() throws Exception {
        Session session = SessionFactory.INSTANCE.createSession(new BattleShipConfiguration(), new Player("test"));
        assertNotNull(session);
    }

    @Test
    public void SessionFactory_ShouldNotReturnNull_WhenCreatingKnownSessionType() throws Exception {
        Session session = SessionFactory.INSTANCE.createSession(GameState.class, new BattleShipConfiguration(), new Player("test"));
        assertNotNull(session);
    }

    @Test(expected = Exception.class)
    public void SessionFactory_ShouldThrowException_OnUnknownSessionType() throws Exception {
        Session session = SessionFactory.INSTANCE.createSession(Object.class, new BattleShipConfiguration(), new Player("test"));
    }
}
